...
def product_offer(request, product_id, offer_id):
    if request.user.is_superuser:
        try:
            ...
            product.offer = offer
            product.save()
            request.session["product_offer_message"] = (
                f"{offer.name} Offer applied to Product {product.name}"
            )
        except ObjectDoesNotExist:
        ...
        try:
            ...
            if Product.objects.filter(category=category_id):
                ...
                if flag == 1:
                    ...
                    category.save()
                    request.session["category_offer_message"] = (
                        f"{offer.name} Offer applied to {category.name} Category"
                    )
                else:
